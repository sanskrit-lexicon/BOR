"""
Markup fixer + audit for bor.txt (BOR).
Counterpart of PWG#175, PWK#113, and siblings.

BOR-specific notes: 26,056 trailing spaces in <div>.

Inputs: ../../../csl-orig/v02/bor/bor.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "bor" / "bor.txt", HERE / "bor.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "bor_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(div|ls)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for bor.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"BOR markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against bor.txt.\n")
        f.write(f"Paired tags: <div>, <ls>\n")
        f.write(f"Notes: 26,056 trailing spaces in <div>.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <div>, <ls>\n\n")
